...
mod imp {
    ...
    use super::paths::{CefRuntimePathConfig, resolve_cef_runtime_paths};
    ...
    fn load_cef_library(runtime_paths: &CefRuntimePathConfig) -> Result<CefLibraryLoader> {
        ...
        let loader_exe = super::paths::macos_loader_executable(
            &runtime_paths.root_cache_path,
            &runtime_paths.executable_path,
        );
        let helper_process =
            super::paths::macos_is_helper_executable(&runtime_paths.executable_path);
        let loader_macos_dir = loader_exe
            .parent()
            .context("CEF loader executable path has no parent directory")?;
        let loader_frameworks_dir = super::paths::macos_frameworks_dir(
            &runtime_paths.root_cache_path,
            &runtime_paths.executable_path,
        );
        ...
        if !helper_process {
            ...
            super::macos_helpers::create_clean_directory(&loader_frameworks_dir)?;
            super::macos_helpers::replace_symlink(
                &loader_frameworks_dir.join("Chromium Embedded Framework.framework"),
                framework_dir,
            )?;
            super::macos_helpers::prepare_macos_helper_apps(runtime_paths, &loader_frameworks_dir)?;
        }

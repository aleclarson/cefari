...
fn render_support_typescript_const(capabilities: &[Capability]) -> String {
    let typescript = render_support_typescript(capabilities);
    format!("pub const IPC_CAPABILITY_SUPPORT_TYPESCRIPT: &str = {typescript:?};\n\n")
}

fn render_support_typescript(capabilities: &[Capability]) -> String {
    let mut output = String::from(
        "// @generated by crates/cefari-core/build.rs; do not edit by hand.\n\
         ...
         \x20 name: string;\n\
         \x20 support: PlatformSupport;\n\
         \x20 targets: readonly CefariTarget[];\n\
         \x20 rationale: string;\n\
         };\n\
         \n\
    ...
    output.push_str(
        "] as const satisfies readonly IpcCapabilitySupport[];\n\
         ...
         \x20 return ipcCapabilitySupport.find((capability) => capability.name === name);\n\
         }\n\
         \n\
         export function supportsTarget(name: string, target: CefariTarget): boolean {\n\
         \x20 return capabilitySupport(name)?.targets.includes(target) ?? false;\n\
         }\n",
    );
    output
}

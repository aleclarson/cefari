...
export type LogScope = "app" | "cefari" | "daemon" | `worker:${string}` | (string & {});
...
const cefariLogDatabaseEnv = "CEFARI_LOG_DATABASE";
...
const secretKeys = new Set(["authorization", "cefari_session_token", "token"]);
const envSecretFragments = ["AUTH", "KEY", "SECRET", "TOKEN"];
const ulidAlphabet = "0123456789ABCDEFGHJKMNPQRSTVWXYZ";

export function getCefariLogDatabasePath(): string {
  const configured = process.env[cefariLogDatabaseEnv];
  if (configured !== undefined && configured.trim() !== "") {
    return configured;
  }

  if (process.platform === "darwin") {
    return join(homedir(), "Library", "Application Support", "dev.Cefari.Cefari", "logs", "cefari.sqlite");
  }

  if (process.platform === "win32") {
    const appData = process.env.APPDATA ?? join(homedir(), "AppData", "Roaming");
    return join(appData, "dev", "Cefari", "Cefari", "data", "logs", "cefari.sqlite");
  }

  const dataHome = process.env.XDG_DATA_HOME ?? join(homedir(), ".local", "share");
  return join(dataHome, "dev", "Cefari", "Cefari", "logs", "cefari.sqlite");
}

export function createLogStore(options: { databasePath?: string; inlineByteLimit?: number } = {}): LogStore {
  const databasePath = options.databasePath ?? getCefariLogDatabasePath();
  ...
  function collapseString(value: string): string {
  ...
  function collapseJsonValue(kind: "array" | "object", value: unknown): unknown {
  ...
  function insertCollapsedValue(kind: LogCollapsedKind, value: unknown, byteLength: number): string {
  ...
  return {
    ...
    retainSince(since) {
      ...
      try {
        db.prepare("DELETE FROM log_entries WHERE at < ?").run(sinceDate);
        db.prepare(
...
export function toErrorProperties(error: unknown): LogProperties {
  return {
...
function isCollapsedValueId(value: string): boolean {
...
function filterByRegex(entries: LogEntry[], pattern: string): LogEntry[] {
  const regex = new RegExp(pattern);
  return entries.filter((entry) => regex.test(`${entry.message} ${JSON.stringify(entry.properties)}`));
}

function toJsonPath(key: string): string {
  return `$."${key.replaceAll('"', '\\"')}"`;
}

function toLogLevelRankSql(column: string): string {
...
function toCollapsedPrefix(kind: LogCollapsedKind): string {
  if (kind === "array") {
    return "arr";
  }
